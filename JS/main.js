...
fadeEls.forEach(function (fadeEls, index) {
   // gsap.to(요소, 지속시간, 옵션);
    gsap.to(fadeEls, 1, {
        delay: (index + 1) * .5, // 0.7, 1.4, 2.1, 2.8 
        opacity: 1 
    });
});
...
new Swiper ('.promotion .swiper-container' , {
    ...
    pagination: {
        el: '.promotion .swiper-pagination', // 페이지 제어
        clickable: true // 사용자가 페이지 번호 제어
    },
    navigation: {
...
spyEls.forEach(function (spyEl) {
    new ScrollMagic.Scene({
            ...
            triggerHook: .8 // .8에 닿았을 때 감시판단
        }).setClassToggle(spyEl, 'show').addTo(new ScrollMagic.Controller());
});

const thisYear = document.querySelector('.this-year');
thisYear.textContent = new Date().getFullYear(); // 당해년도